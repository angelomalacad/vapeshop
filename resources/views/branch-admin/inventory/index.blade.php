...
@section('content')
    <div class="container-fluid">
        ...
        <ul class="nav nav-tabs mb-4" id="inventoryTabs" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="my-branch-tab" data-bs-toggle="tab" data-bs-target="#my-branch" type="button" role="tab">
                    <i class="bi bi-shop me-1"></i> My Branch ({{ Auth::user()->branch->name }})
                </button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="other-branches-tab" data-bs-toggle="tab" data-bs-target="#other-branches" type="button" role="tab">
                    <i class="bi bi-building me-1"></i> Other Branches
                </button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="disposed-tab" data-bs-toggle="tab" data-bs-target="#disposed" type="button" role="tab">
        ...
        <div class="tab-content" id="inventoryTabsContent">
            ...
            <div class="tab-pane fade show active" id="my-branch" role="tabpanel">
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <form method="GET" class="row g-3" id="myBranchForm" action="{{ route('branch-admin.inventory.index') }}">
                            ...
                            <div class="col-12">
                                ...
                                <a href="{{ route('branch-admin.inventory.index', ['tab' => 'my-branch']) }}" class="btn btn-outline-secondary ms-2">
                                ...
                                <tbody>
                                    @forelse($inventories as $inv)
                                        ...
                                        <tr>
                                            <td style="width: 60px">
                                                @if($imageUrl)
                                                    <img src="{{ $imageUrl }}" alt="{{ $product->name }}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 8px;">
                                                @else
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm" role="group">
                                                    <button type="button" class="btn btn-outline-info" title="View Details" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-url="{{ route('branch-admin.inventory.show-modal', $inv) }}">
                                                        <i class="bi bi-eye"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-warning" title="Edit Inventory" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-url="{{ route('branch-admin.inventory.edit-modal', $inv) }}">
                                                        <i class="bi bi-pencil"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-outline-primary" title="Transfer Stock" data-bs-toggle="modal" data-bs-target="#dynamicModal" data-url="{{ route('branch-admin.inventory.transfer-modal', ['inventory_id' => $inv->id]) }}">
                                                        <i class="bi bi-arrow-left-right"></i>
                                                    </button>
                                                    
                                                    @if($isArchived)
                                                        <form action="{{ route('branch-admin.inventory.unarchive', $inv) }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <button type="submit" class="btn btn-outline-success" title="Restore Item" onclick="return confirm('Restore this item back to inventory?')">
                                                                <i class="bi bi-arrow-counterclockwise"></i>
                                                            </button>
                                                        </form>
                                                    @else
                                                        <button type="button" class="btn btn-outline-secondary archive-btn" title="Archive Item" data-bs-toggle="modal" data-bs-target="#archiveModal" data-id="{{ $inv->id }}" data-name="{{ $product->name }}">
                                                            <i class="bi bi-archive"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-outline-danger dispose-btn" title="Dispose Item" data-bs-toggle="modal" data-bs-target="#disposeModal" data-id="{{ $inv->id }}" data-name="{{ $product->name }}">
                    ...
                    <div class="card-footer bg-white">
                        <div class="d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                ...
                                @else
                                    <a href="{{ $inventories->previousPageUrl() }}&tab=my-branch" class="btn btn-outline-primary rounded-pill px-3 me-2">
                                        Previous
                                    </a>
                                @endif

                                @if ($inventories->hasMorePages())
                                    <a href="{{ $inventories->nextPageUrl() }}&tab=my-branch" class="btn btn-outline-primary rounded-pill px-3">
                                        Next
                                    </a>
                                @else
...
<div class="tab-pane fade" id="other-branches" role="tabpanel">
    <div class="card mb-4">
        ...
        <div class="card-body">
            ...
            <form method="GET" class="row g-3 mb-4" id="otherBranchesForm" action="{{ route('branch-admin.inventory.index') }}">
                <input type="hidden" name="tab" value="other-branches">
                <div class="col-md-4">
                    <label class="form-label">Search Product</label>
                    <input type="text" name="search_other" class="form-control" placeholder="Search by product name..." value="{{ request('search_other') }}">
                </div>
                <div class="col-md-4">
                    ...
                    <select name="branch_filter" class="form-select">
                        ...
                        @php
                            ...
                            $allOtherBranches = \App\Models\Branch::where('id', '!=', $currentBranchId)->get();
                        @endphp
                ...
                <div class="col-md-4">
                    ...
                    <select name="product_id_other" class="form-select">
                        ...
                        @foreach($allProducts as $product)
                            <option value="{{ $product->id }}" {{ request('product_id_other') == $product->id ? 'selected' : '' }}>
                                {{ $product->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-12">
                    ...
                    <a href="{{ route('branch-admin.inventory.index', ['tab' => 'other-branches']) }}" class="btn btn-outline-secondary ms-2">
                        <i class="bi bi-arrow-counterclockwise"></i> Reset
                    </a>
                </div>
            </form>
                            ...
                            <tbody>
                                @foreach($branchItems as $inv)
                                    ...
                                    <tr>
                                        ...
                                        <td>₱{{ number_format($product->price, 2) }}</td>
                                        <td>
                                            <button type="button" class="btn btn-sm btn-primary request-transfer-btn" 
                    ...
                    <div class="card-body">
                        ...
                        @if($disposedItems->count() > 0)
                                    ...
                                    <tbody>
                                        @foreach($disposedItems as $item)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $item->quantity }}</td>
                                                <td>{{ $item->disposed_at ? $item->disposed_at->format('M d, Y h:i A') : $item->updated_at->format('M d, Y h:i A') }}</td>
                                                <td>{{ $item->dispose_reason ?? 'No reason provided' }}</td>
                                                <td>
                                                    <form action="{{ route('branch-admin.inventory.restore-disposed', $item) }}" method="POST" class="d-inline">
                                                        @csrf
                                                        <button type="submit" class="btn btn-sm btn-outline-success" onclick="return confirm('Restore this item back to inventory?')">
                            ...
                            <div class="d-flex justify-content-center mt-3">
                                ...
                                @else
                                    <a href="{{ $disposedItems->previousPageUrl() }}&tab=disposed" class="btn btn-outline-primary rounded-pill px-3 me-2">
                                        Previous
                                    </a>
                                @endif

                                @if ($disposedItems->hasMorePages())
                                    <a href="{{ $disposedItems->nextPageUrl() }}&tab=disposed" class="btn btn-outline-primary rounded-pill px-3">
                                        Next
                                    </a>
                                @else
                                    <span class="btn btn-outline-secondary disabled rounded-pill px-3">
                ...
                <form id="disposeForm" method="POST" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="dispose_reason" class="form-control" rows="3" placeholder="e.g., Expired, Damaged, Defective, etc."></textarea>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            requestTransferBtns.forEach(button => {
                button.addEventListener('click', function(e) {
                    ...
                    const url = '/branch-admin/inventory/transfer-modal?inventory_id=' + inventoryId + '&from_branch=' + fromBranch;
