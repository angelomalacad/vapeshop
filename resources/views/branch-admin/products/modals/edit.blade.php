...
<div class="modal fade" id="editProductModal{{ $product->id }}" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            ...
            <form method="POST" action="{{ route('branch-admin.products.update', $product) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <select name="brand" class="form-select" required>
                                                ...
                                                <option value="Other" {{ !in_array($product->brand, ['X-Vape', 'Slimbar', 'Relx']) ? 'selected' : '' }}>Other</option>
                                            </select>
                                        </div>
                                    </div>

                                    <div class="row" id="customBrandRow{{ $product->id }}" style="display: {{ !in_array($product->brand, ['X-Vape', 'Slimbar', 'Relx']) ? 'block' : 'none' }};">
                                        <div class="col-md-12 mb-3">
                                            <label class="form-label">Custom Brand Name</label>
                                            <input type="text" name="custom_brand" class="form-control" value="{{ !in_array($product->brand, ['X-Vape', 'Slimbar', 'Relx']) ? $product->brand : '' }}" placeholder="Enter custom brand name">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <select name="category" class="form-select" required>
                                                ...
                                                <option value="New" {{ !in_array($product->category, ['Ultra', 'Slimbar', 'Relx']) ? 'selected' : '' }}>New Category</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4 mb-3" id="newCategoryRow{{ $product->id }}" style="display: {{ !in_array($product->category, ['Ultra', 'Slimbar', 'Relx']) ? 'block' : 'none' }};">
                                            <label class="form-label">New Category Name</label>
                                            <input type="text" name="new_category" class="form-control" value="{{ !in_array($product->category, ['Ultra', 'Slimbar', 'Relx']) ? $product->category : '' }}" placeholder="Enter new category name">
                                        </div>
                                        <div class="col-md-4 mb-3">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-4 mb-3">
                                            ...
                                            <input type="text" name="nicotine_strength" class="form-control" value="{{ $product->nicotine_strength }}" placeholder="e.g., 10mg">
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Puff Count</label>
                                            <input type="number" name="puff_count" class="form-control" value="{{ $product->puff_count }}" placeholder="e.g., 10000">
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Battery Capacity (mAh)</label>
                                            <input type="number" name="battery_capacity" class="form-control" value="{{ $product->battery_capacity }}" placeholder="e.g., 650">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Charging Type</label>
                                            <input type="text" name="charging_type" class="form-control" value="{{ $product->charging_type }}" placeholder="e.g., Type-C">
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <label class="form-label">Liquid Capacity (ml)</label>
                                            <input type="number" step="0.1" name="liquid_capacity" class="form-control" value="{{ $product->liquid_capacity }}" placeholder="e.g., 10">
                                        </div>
                                        <div class="col-md-4 mb-3">
                                            <div class="form-check mt-4">
                                                <input class="form-check-input" type="checkbox" id="adjustable_airflow{{ $product->id }}" name="adjustable_airflow" value="1" {{ $product->adjustable_airflow ? 'checked' : '' }}>
                                                <label class="form-check-label" for="adjustable_airflow{{ $product->id }}">
                                                    Adjustable Airflow
                                                </label>
                                            </div>
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" id="smart_display{{ $product->id }}" name="smart_display" value="1" {{ $product->smart_display ? 'checked' : '' }}>
                                                <label class="form-check-label" for="smart_display{{ $product->id }}">
                                                    Smart Display
                                                </label>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <div class="input-group">
                                                <span class="input-group-text">₱</span>
                                                <input type="number" step="0.01" name="cost" class="form-control" value="{{ $product->cost }}" placeholder="0.00">
                                            </div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-3 text-center" id="newStoragePreview{{ $product->id }}" style="display: none;">
                                        <img id="storagePreviewImg{{ $product->id }}" src="" alt="Preview" style="max-height: 150px;" class="img-thumbnail">
                                        <p class="text-muted small mt-1">New image preview</p>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <input type="file" name="image" class="form-control" accept="image/*" id="storageFileInput{{ $product->id }}">
                                        <small class="text-muted">Upload new image from computer (max 2MB)</small>
                                        <small class="text-muted d-block text-warning">Note: This will replace any existing Google Drive image</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="mb-3 text-center" id="newDrivePreview{{ $product->id }}" style="display: none;">
                                        <img id="drivePreviewImg{{ $product->id }}" src="" alt="Preview" style="max-height: 150px;" class="img-thumbnail">
                                        <p class="text-muted small mt-1">Google Drive image preview</p>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <div class="input-group">
                                            <input type="text" name="image_url" class="form-control" placeholder="Paste Google Drive shareable link" value="{{ $product->image_url }}" id="googleDriveUrl{{ $product->id }}">
                                            <button class="btn btn-outline-primary" type="button" onclick="convertGoogleDriveUrl({{ $product->id }})">
                                                <i class="bi bi-google"></i> Use
                                            </button>
                                        </div>
                                        <small class="text-muted">Paste Google Drive shareable link</small>
                                        <small class="text-muted d-block text-warning">Note: This will replace any existing storage image</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    @if($branchInventory)
                                        ...
                                        <div class="mb-2">
                                            <strong>Low Stock Threshold:</strong> {{ $branchInventory->low_stock_threshold }} units
                                        </div>
                                        <div class="mb-2">
                                            <strong>Last Restocked:</strong> {{ $branchInventory->last_restocked_at ? $branchInventory->last_restocked_at->format('M d, Y') : 'Never' }}
                                        </div>
                                    @else
                                        <p class="text-muted mb-0">Product not in inventory</p>
                                        <a href="{{ route('branch-admin.inventory.add-to-inventory', $product->id) }}" class="btn btn-sm btn-primary mt-2">
        ...
        storageFileInput{{ $product->id }}.addEventListener('change', function(e) {
            ...
            if (file) {
                ...
                reader.onload = function(event) {
                    ...
                    document.getElementById('newStoragePreview{{ $product->id }}').style.display = 'block';
                    // Hide current storage preview if exists
                    const currentPreview = document.getElementById('currentStoragePreview{{ $product->id }}');
                    if (currentPreview) {
...
<script>
    const brandSelect{{ $product->id }} = document.querySelector('#editProductModal{{ $product->id }} select[name="brand"]');
    ...
    const categorySelect{{ $product->id }} = document.querySelector('#editProductModal{{ $product->id }} select[name="category"]');
