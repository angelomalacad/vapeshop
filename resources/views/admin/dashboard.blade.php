...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        @keyframes pulseGlow {
            0%, 100% { box-shadow: 0 0 20px rgba(13,110,253,0.15); transform: scale(1); }
            50% { box-shadow: 0 0 40px rgba(13,110,253,0.3); transform: scale(1.03); }
        }
        ...
        .animate-fade-left { animation: fadeInLeft 0.5s ease forwards; opacity: 0; }
        .animate-fade-right { animation: fadeInRight 0.5s ease forwards; opacity: 0; }
        ...
        .sidebar-card .list-group-item:hover::before { left: 100%; }
        ...
        .sidebar-card .list-group-item i { width: 24px; color: #6c757d; transition: transform 0.2s ease; }
        .sidebar-card .list-group-item:hover i { transform: scale(1.1); color: #0d6efd; }
        ...
        .stat-card:hover { transform: translateY(-6px); box-shadow: 0 20px 30px -12px rgba(0,0,0,0.12); }
        ...
        .low-stock-badge { animation: gentlePulse 2s infinite; display: inline-block; }
        ...
        .card:hover { box-shadow: 0 12px 24px rgba(0,0,0,0.08); }
        .card-header {
            background: white;
        ...
        .nav-tabs .nav-link.active::before { width: 60%; }
        ...
        /* Chart containers */
        .chart-container-small { max-width: 100%; height: 200px; margin: 0 auto; }
        .chart-container-medium { max-width: 100%; height: 250px; margin: 0 auto; }
        .chart-wrapper { position: relative; width: 100%; min-height: 200px; }
        canvas { max-height: 100%; width: 100% !important; transition: all 0.3s ease; }
        ...
        .table tbody tr:hover { background-color: rgba(13,110,253,0.03); transform: scale(1.01); }
        
        .progress { border-radius: 10px; background-color: #e9ecef; overflow: hidden; }
        ...
        @media (max-width: 768px) {
            ...
            .row > .col-md-3.sidebar-open { left: 0; }
            .sidebar-card { border-radius: 0; height: 100%; overflow-y: auto; }
            .sidebar-close-btn { display: block; }
            .mobile-menu-toggle { display: flex; align-items: center; justify-content: center; }
            .sidebar-overlay.active { display: block; opacity: 1; }
            .row > .col-md-9 { width: 100%; }
            .navbar .container { flex-wrap: wrap; gap: 0.5rem; }
            .navbar-nav { flex-direction: row; align-items: center; gap: 0.5rem; }
            .navbar-text { font-size: 0.8rem; margin-right: 0.5rem !important; }
            .btn-outline-light { padding: 0.25rem 0.75rem; font-size: 0.8rem; }
            .stat-card h2 { font-size: 1.2rem; }
            .stat-card h5 { font-size: 0.6rem; }
            .stat-icon { width: 35px; height: 35px; }
            .stat-icon i { font-size: 1rem !important; }
            .welcome-banner .col-md-8,
            .welcome-banner .col-md-4 { text-align: center !important; }
            .welcome-banner .text-md-end { text-align: center !important; margin-top: 0.75rem; }
            ...
            .table th, .table td { padding: 0.5rem; font-size: 0.75rem; white-space: nowrap; }
        }
        
        @media (max-width: 480px) {
            .stat-card .card-body { padding: 0.75rem; }
            .stat-card h2 { font-size: 1rem; }
            .welcome-banner .bg-white.bg-opacity-10 { padding: 0.5rem 0.75rem !important; font-size: 0.7rem; }
        ...
        @media (max-width: 768px) {
            .list-group-item, .nav-link, .btn, .stat-card {
                cursor: pointer;
                -webkit-tap-highlight-color: rgba(13,110,253,0.1);
            }
        }
        /* ========== EQUAL HEIGHT CARDS FIX ========== */
/* Make all cards in the same row have equal height */
.row.g-4 {
    display: flex;
    flex-wrap: wrap;
}
...
/* Ensure Today's Activity badges wrap properly */
.row.g-4 .badge {
    white-space: nowrap;
}
...
<body>
    ...
    <nav class="navbar navbar-expand-lg animate-fade-up" style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%);">
        <div class="container">
            <a class="navbar-brand text-white fw-bold" href="{{ route('home') }}">
                <img src="{{ asset('images/logo.png') }}" alt="Vape Expo Logo" height="30" class="d-inline-block align-text-top me-2" onerror="this.style.display='none'">
        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                if (class_exists('\App\Models\Order') && \App\Models\Order::count() > 0) {
                    ...
                    $posCompleted = \App\Models\Order::where('order_number', 'LIKE', 'POS-%')->where('status', 'completed')->count();
                    $posCancelled = \App\Models\Order::where('order_number', 'LIKE', 'POS-%')->where('status', 'cancelled')->count();
                }
                ...
                if (class_exists('\App\Models\Order') && \App\Models\Order::count() > 0) {
                    ...
                    $onlinePending = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'pending')->count();
                    $onlineConfirmed = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'confirmed')->count();
                    $onlineProcessing = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'processing')->count();
                    $onlineReady = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'ready')->count();
                    $onlineOutForDelivery = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'out_for_delivery')->count();
                    $onlineDelivered = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'delivered')->count();
                    $onlineCancelled = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->where('order_status', 'cancelled')->count();
                }
                ...
                if (class_exists('\App\Models\DriverShift') && \App\Models\DriverShift::count() > 0) {
                    ...
                    $activeDriversToday = \App\Models\DriverShift::where('status', 'active')->whereDate('shift_date', today())->count();
                    $pendingShifts = \App\Models\DriverShift::where('status', 'pending')->whereDate('shift_date', today())->count();
                    $completedShifts = \App\Models\DriverShift::where('status', 'completed')->whereDate('shift_date', today())->count();
                }
                ...
                if (class_exists('\App\Models\Order') && \App\Models\Order::count() > 0) {
                    ...
                    $posToday = \App\Models\Order::where('order_number', 'LIKE', 'POS-%')->whereDate('created_at', today())->count();
                    $onlineToday = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->whereDate('created_at', today())->count();
                }
...
if (class_exists('\App\Models\DriverShift') && \App\Models\DriverShift::count() > 0) {
    $activeDriversList = \App\Models\DriverShift::with('driver')
        ...
        ->toArray();
                ...
                if (class_exists('\App\Models\Order') && \App\Models\Order::count() > 0) {
                    $deliveryVsPickup['delivery_sales'] = \App\Models\Order::where('order_number', 'LIKE', 'ORD-%')->sum('total_amount') ?? 0;
                    $deliveryVsPickup['pickup_sales'] = \App\Models\Order::where('order_number', 'LIKE', 'POS-%')->sum('total_amount') ?? 0;
                }
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane fade show active" id="overview" role="tabpanel">
                                ...
                                <div class="card stat-card">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <h5>LOW STOCK</h5>
                                                <h2 class="{{ $lowStockCount > 0 ? 'text-warning' : '' }}">{{ $lowStockCount }}</h2>
                                                @if($lowStockCount > 0)
                                ...
                                <div class="card stat-card">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                <h5>OUT OF STOCK</h5>
                                                <h2 class="{{ $outOfStockCount > 0 ? 'text-danger' : '' }}">{{ $outOfStockCount }}</h2>
                                    ...
                                    <div class="card-body">
                                        <div class="d-flex justify-content-between align-items-start">
                                            <div>
                                                ...
                                                <h2 class="{{ $pendingTransfers > 0 ? 'text-warning' : '' }}">{{ $pendingTransfers }}</h2>
                        ...
                        <!-- Row 3: Warehouse, POS Orders, Online Orders, Deliveries -->
<div class="row g-4 mb-4">
    <!-- Warehouse Stock -->
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <a href="{{ route('admin.warehouse.index') }}" class="small text-decoration-none mt-2 d-inline-block text-primary">View Stock →</a>
                    </div>
                    <div class="stat-icon" style="background: rgba(13,110,253,0.1); color:#0d6efd;">
                ...
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <div class="small text-muted">
                            <span class="badge bg-success bg-opacity-10 text-success">{{ $posCompleted }} completed</span>
                            @if($posCancelled > 0)
                            <span class="badge bg-danger bg-opacity-10 text-danger ms-1">{{ $posCancelled }} cancelled</span>
                            @endif
                        </div>
                        <a href="{{ route('admin.pos.history') }}" class="small text-decoration-none mt-2 d-inline-block text-primary">View POS →</a>
                    </div>
                    <div class="stat-icon" style="background: rgba(25,135,84,0.1); color:#198754;">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <div class="small text-muted">
                            <span class="badge bg-warning bg-opacity-10 text-warning">{{ $onlinePending }} pending</span>
                            <span class="badge bg-info bg-opacity-10 text-info ms-1">{{ $onlineConfirmed }} confirmed</span>
                            <span class="badge bg-success bg-opacity-10 text-success ms-1">{{ $onlineDelivered }} delivered</span>
                        </div>
                        <a href="{{ route('admin.online-orders.index') }}" class="small text-decoration-none mt-2 d-inline-block text-primary">View Orders →</a>
                    </div>
                    <div class="stat-icon" style="background: rgba(13,110,253,0.1); color:#0d6efd;">
            ...
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        <div class="small text-muted">
                            <span class="badge bg-warning bg-opacity-10 text-warning">{{ $pendingDeliveries }} pending</span>
                            <span class="badge bg-info bg-opacity-10 text-info ms-1">{{ $inTransitDeliveries }} in transit</span>
                            <span class="badge bg-success bg-opacity-10 text-success ms-1">{{ $deliveredDeliveries }} delivered</span>
                        </div>
                        <div class="progress mt-2" style="height: 4px;">
                            <div class="progress-bar bg-success" role="progressbar" style="width: {{ $deliverySuccessRate }}%;"></div>
                        </div>
                        <div class="small text-muted mt-1">Success rate: {{ $deliverySuccessRate }}%</div>
                        <a href="{{ route('admin.deliveries.index') }}" class="small text-decoration-none mt-2 d-inline-block text-primary">View All →</a>
                    </div>
                    <div class="stat-icon" style="background: rgba(13,110,253,0.1); color:#0d6efd;">
                       ...
                       <!-- Row 4: Driver Shifts, Today's Orders, Today's POS, Today's Online -->
<div class="row g-4 mb-4">
    <!-- Driver Shifts -->
    <div class="col-md-3 col-6 animate-scale delay-14">
        <div class="card stat-card">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        ...
                        @if(count($activeDriversList) > 0)
                            <div class="fw-bold">{{ implode(', ', $activeDriversList) }}</div>
                            <div class="small text-success">active now</div>
                        @else
                            <div class="small text-muted">No active drivers</div>
                        @endif
                        <a href="{{ route('admin.driver-shifts.index') }}" class="small text-decoration-none mt-2 d-inline-block text-primary">Manage →</a>
                    </div>
                    <div class="stat-icon" style="background: rgba(25,135,84,0.1); color:#198754;">
    ...
    <!-- Today's Activity - Right Column -->
<!-- Today's Activity - Right Column (Simpler Horizontal) -->
<div class="col-md-6 col-12 animate-scale delay-15">
            ...
            <div class="d-flex justify-content-between align-items-start">
                <div class="flex-grow-1">
                    ...
                    <div class="small text-muted mt-3 text-center">
                        <i class="bi bi-calendar-check me-1"></i> {{ now()->format('l, F j, Y') }}
                    </div>
                </div>
                <div class="stat-icon ms-3" style="background: rgba(13,110,253,0.1); color:#0d6efd; min-width: 45px;">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <ul>
                                            ...
                                            <li><strong>Branch Personnel</strong> - {{ $totalStaff }} staff members</li>
                                            <li><strong>Product Catalog</strong> - {{ $totalProducts }} products</li>
                                            <li><strong>Warehouse Stock</strong> - {{ number_format($totalWarehouseStock) }} units</li>
                                            <li><strong>Total Orders</strong> - {{ $totalAllOrders }} ({{ $totalPosOrders }} POS + {{ $totalOnlineOrders }} Online)</li>
                                        </ul>
                                    </div>
                                    <div class="col-md-6">
                                        <ul>
                                            <li><strong>Active Deliveries</strong> - {{ $activeDeliveries }} in progress</li>
                                            <li><strong>Active Drivers Today</strong> - {{ $activeDriversToday }} drivers on duty</li>
                                            ...
                                            <li><strong>Delivery Success Rate</strong> - {{ $deliverySuccessRate }}%</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                <div class="card analytics-card h-100 animate-fade-up delay-3">
                                    ...
                                    <div class="card-body text-center">
                                        ...
                                        @else
                                        <div class="chart-fallback"><p class="text-muted">No sales data yet</p></div>
                                        @endif
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        tabButtons.forEach(button => {
            button.addEventListener('shown.bs.tab', function(e) {
                ...
                const animatedElements = targetPane.querySelectorAll('.animate-scale, .animate-fade-up, .animate-fade-left, .animate-fade-right');
                animatedElements.forEach(el => {
                    const classes = el.className;
                    el.classList.remove('animate-scale', 'animate-fade-up', 'animate-fade-left', 'animate-fade-right');
                    ...
                    if (classes.includes('animate-fade-left')) el.classList.add('animate-fade-left');
                    if (classes.includes('animate-fade-right')) el.classList.add('animate-fade-right');
                });
