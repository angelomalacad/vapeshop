        ...
        <form id="addStockForm" method="POST" action="{{ route('admin.products.add-stock', $product) }}">
            ...
            <div class="mb-3">
                ...
                <div class="form-text" id="destinationHelp">
                    ...
                    <span id="helpText">Select "Main Warehouse" to add new stock, or select a branch to transfer from
                        warehouse</span>
                </div>
            </div>
            ...
            <div id="warehouseStockInfo" class="alert alert-info mb-3" style="display: none;">
                ...
                <div id="lowStockWarning" class="small text-warning mt-1" style="display: none;">
                    <i class="bi bi-exclamation-triangle-fill me-1"></i> Low stock! Only <span id="lowStockQty"></span>
                ...
                if (data.success) {
                    ...
                    if (currentWarehouseStock === 0) {
                        ...
                        quantityHelp.innerHTML =
                            '<span class="text-danger">⚠️ No stock available in warehouse. Please add stock to warehouse first.</span>';
                        quantityInput.max = 0;
                    } else if (currentWarehouseStock < 10) {
                        lowStockWarning.style.display = 'block';
                        lowStockQty.innerHTML = currentWarehouseStock;
                        quantityHelp.innerHTML =
                            `<span class="text-warning">⚠️ Only ${currentWarehouseStock} units available in warehouse.</span>`;
                        quantityInput.max = currentWarehouseStock;
                    } else {
                        quantityHelp.innerHTML =
                            `<span class="text-success">✓ ${currentWarehouseStock} units available in warehouse</span>`;
                        quantityInput.max = currentWarehouseStock;
                    }
    ...
    function updateUI() {
        ...
        if (branchId === 'warehouse') {
            ...
            helpTextSpan.innerHTML =
                'Transferring stock from Main Warehouse to selected branch. Stock will be deducted from warehouse.';
            expiryHelp.innerHTML = 'Optional - will be copied from warehouse if available';
            checkWarehouseStock();
        } else {
            // No selection
            helpTextSpan.innerHTML =
                'Select "Main Warehouse" to add new stock, or select a branch to transfer from warehouse';
    ...
    function validateQuantity() {
        ...
        if (branchId && branchId !== 'warehouse') {
            if (quantity > currentWarehouseStock) {
                ...
                alertDiv.innerHTML =
                    `<i class="bi bi-exclamation-triangle-fill me-2"></i> Insufficient warehouse stock. Available: ${currentWarehouseStock} units.`;
            ...
            if (quantity <= 0) {
                alertDiv.className = 'alert alert-danger';
                alertDiv.innerHTML =
                    '<i class="bi bi-exclamation-triangle-fill me-2"></i> Please enter a valid quantity.';
                alertDiv.style.display = 'block';
                return false;
            }
        }
        ...
        if (branchId === 'warehouse' && !expirationDate.value) {
            if (confirm(
                    'No expiration date set for warehouse stock. This is recommended for inventory tracking. Continue anyway?'
                )) {
                return true;
            }
            return false;
    ...
    quantityInput.addEventListener('input', function() {
        ...
        if (branchId && branchId !== 'warehouse') {
            if (quantity > currentWarehouseStock) {
                ...
                quantityHelp.innerHTML =
                    `<span class="text-success">✓ Transferring ${quantity} units from warehouse</span>`;
            }
        }
    });
        ...
        fetch(form.action, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alertDiv.className = 'alert alert-danger';
                    let errorHtml =
                        '<i class="bi bi-exclamation-triangle-fill me-2"></i> <strong>Error:</strong><ul>';
                    if (data.errors) {
