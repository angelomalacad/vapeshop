...
@section('title', 'Edit Product - Vape Expo')
        ...
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                <p class="text-muted mb-0">
                    <i class="bi bi-pencil me-1"></i> {{ $product->name }}
                </p>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="brand" class="form-select @error('brand') is-invalid @enderror" id="brand" required>
                            ...
                            <option value="X-Vape" {{ old('brand', $product->brand) == 'X-Vape' ? 'selected' : '' }}>X-Vape</option>
                            <option value="Slimbar" {{ old('brand', $product->brand) == 'Slimbar' ? 'selected' : '' }}>Slimbar</option>
                            <option value="Relx" {{ old('brand', $product->brand) == 'Relx' ? 'selected' : '' }}>Relx</option>
                ...
                <div class="row" id="customBrandRow" style="display: {{ old('brand', $product->brand) == 'Other' ? 'block' : 'none' }};">
                    <div class="col-md-6 mb-3">
                        <label class="form-label fw-semibold">Custom Brand Name</label>
                        <input type="text" name="custom_brand" class="form-control" value="{{ old('custom_brand', $product->brand) }}">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="category" class="form-select @error('category') is-invalid @enderror" id="category" required>
                            ...
                            <option value="Ultra" {{ old('category', $product->category) == 'Ultra' ? 'selected' : '' }}>X-Vape Ultra</option>
                            <option value="Slimbar" {{ old('category', $product->category) == 'Slimbar' ? 'selected' : '' }}>Slimbar</option>
                            <option value="Relx" {{ old('category', $product->category) == 'Relx' ? 'selected' : '' }}>Relx</option>
                            <option value="New">New Category</option>
                        </select>
                        @error('category') <div class="invalid-feedback">{{ $message }}</div> @enderror
                    </div>
                    <div class="col-md-4 mb-3" id="newCategoryRow" style="display: {{ old('category', $product->category) == 'New' ? 'block' : 'none' }};">
                    ...
                    <div class="col-md-4 mb-3">
                        ...
                        <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                            ...
                            <option value="pod-system" {{ old('type', $product->type) == 'pod-system' ? 'selected' : '' }}>Pod System</option>
                            <option value="disposable" {{ old('type', $product->type) == 'disposable' ? 'selected' : '' }}>Disposable</option>
                            <option value="mod" {{ old('type', $product->type) == 'mod' ? 'selected' : '' }}>Box Mod</option>
                            <option value="liquid" {{ old('type', $product->type) == 'liquid' ? 'selected' : '' }}>E-Liquid</option>
                            <option value="coil" {{ old('type', $product->type) == 'coil' ? 'selected' : '' }}>Coil</option>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="nicotine_strength" class="form-control" value="{{ old('nicotine_strength', $product->nicotine_strength) }}" placeholder="e.g., 10mg, 20mg">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Puff Count</label>
                        <input type="number" name="puff_count" class="form-control" value="{{ old('puff_count', $product->puff_count) }}" placeholder="e.g., 10000">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Battery Capacity (mAh)</label>
                        <input type="number" name="battery_capacity" class="form-control" value="{{ old('battery_capacity', $product->battery_capacity) }}" placeholder="e.g., 650">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <input type="text" name="charging_type" class="form-control" value="{{ old('charging_type', $product->charging_type) }}" placeholder="e.g., Type-C">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Liquid Capacity (ml)</label>
                        <input type="number" step="0.1" name="liquid_capacity" class="form-control" value="{{ old('liquid_capacity', $product->liquid_capacity) }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <div class="form-check mt-4">
                            <input type="checkbox" name="adjustable_airflow" class="form-check-input" id="adjustable_airflow" value="1" {{ old('adjustable_airflow', $product->adjustable_airflow) ? 'checked' : '' }}>
                            <label class="form-check-label" for="adjustable_airflow">Adjustable Airflow</label>
                        </div>
                        <div class="form-check">
                            <input type="checkbox" name="smart_display" class="form-check-input" id="smart_display" value="1" {{ old('smart_display', $product->smart_display) ? 'checked' : '' }}>
                ...
                <div id="flavors-container">
                    ...
                    <div class="flavor-item mb-2">
                        <div class="row">
                            <div class="col-md-5">
                                <input type="text" class="form-control" name="flavors[{{ $index }}][name]" value="{{ $flavor->name }}" placeholder="Flavor name">
                            </div>
                            <div class="col-md-3">
                                <input type="text" class="form-control" name="flavors[{{ $index }}][code]" value="{{ $flavor->code }}" placeholder="Code">
                            </div>
                            <div class="col-md-3">
                                <select class="form-select" name="flavors[{{ $index }}][category]">
                                    <option value="">Category</option>
                                    <option value="fruit" {{ $flavor->category == 'fruit' ? 'selected' : '' }}>Fruit</option>
                                    <option value="mint" {{ $flavor->category == 'mint' ? 'selected' : '' }}>Mint</option>
                                    <option value="tea" {{ $flavor->category == 'tea' ? 'selected' : '' }}>Tea</option>
                                    <option value="dessert" {{ $flavor->category == 'dessert' ? 'selected' : '' }}>Dessert</option>
                                    <option value="tobacco" {{ $flavor->category == 'tobacco' ? 'selected' : '' }}>Tobacco</option>
                                </select>
                            </div>
                            <div class="col-md-1">
                    ...
                    <div class="flavor-item mb-2">
                        <div class="row">
                            <div class="col-md-5">
                                <input type="text" class="form-control" name="flavors[0][name]" placeholder="Flavor name">
                            </div>
                            <div class="col-md-3">
                                <input type="text" class="form-control" name="flavors[0][code]" placeholder="Code">
                            </div>
                            <div class="col-md-3">
                        ...
                        <div class="border rounded p-3 bg-light">
                            <div class="mb-3 text-center">
                                @if($product->image_url)
                                    <img id="imagePreview" src="{{ \App\Helpers\GoogleDriveHelper::getDirectImageUrl($product->image_url) }}" alt="Preview" style="max-height: 200px; object-fit: contain;">
                                @elseif($product->image)
                                    <img id="imagePreview" src="{{ Storage::url($product->image) }}" alt="Preview" style="max-height: 200px; object-fit: contain;">
                                @else
                            ...
                            <div class="tab-content mt-3">
                                ...
                                <div class="tab-pane" id="gdrive">
                                    <input type="url" name="image_url" class="form-control" placeholder="https://drive.google.com/file/d/.../view" value="{{ old('image_url', $product->image_url) }}" onchange="previewGdrive(this)">
                                    <div class="form-check mt-2">
                                        <input type="checkbox" name="remove_image" class="form-check-input" id="remove_image" value="1">
...
@push('scripts')
<script>
    let flavorIndex = {{ $product->flavors->count() }};
