    ...
    <div class="row mb-4">
        ...
        <div class="col-md-4 text-md-end">
            <div class="input-group w-100">
                <input type="text" id="productSearch" class="form-control rounded-pill" placeholder="Search products by name, brand, or flavor...">
                <button class="btn btn-outline-secondary rounded-pill ms-2" id="searchButton"><i class="bi bi-search"></i></button>
            </div>
        </div>
    </div>
            ...
            <div class="col-lg-2 col-md-3 col-4">
                <div class="card product-card h-100" data-product-id="{{ $product->id }}">
                    <div class="position-relative">
                        @if($product->image)
                            <img src="{{ Storage::url($product->image) }}" class="product-img" alt="{{ $product->name }}" style="height: 120px; object-fit: cover;">
                        @else
                            <div class="product-img bg-light d-flex align-items-center justify-content-center" style="height: 120px;">
                    ...
                    <div class="card-body p-2 text-center">
                        ...
                        <div class="product-price small fw-bold text-danger">₱{{ number_format($product->price, 2) }}</div>
                    </div>
            ...
            <div class="card product-card h-100">
                <div class="position-relative">
                    ...
                    @if($firstVariant['image'])
                        <img src="{{ Storage::url($firstVariant['image']) }}" class="product-img" alt="{{ $productName }}">
                    @else
                    ...
                    @php
                        ...
                        // Check stock availability across all branches for this variant
                        $hasStock = $variants->contains(fn($v) => $v['available_quantity'] > 0);
                        $hasLowStock = $variants->contains(fn($v) => $v['available_quantity'] <= 5 && $v['available_quantity'] > 0);
                    @endphp
                    @if(!$hasStock)
                ...
                <form action="{{ route('customer.cart.add') }}" method="POST" id="itemCartForm">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mt-3">
                            ...
                            <input type="number" name="quantity" id="itemQuantity" class="form-control" min="1" value="1">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (itemModalElement) {
            ...
            console.log('Item buttons found:', itemButtons.length);
            ...
            itemButtons.forEach(btn => {
                btn.addEventListener('click', function(e) {
                    ...
                    try {
                        ...
                        alert('Error loading product items. Please refresh the page.');
                        return;
                    }
                    ...
                    itemSelect.innerHTML = '<option value="">Select branch and item...</option>';
                    variants.forEach(variant => {
                        ...
                        option.dataset.branchName = variant.branch_name || 'Unknown Branch';
                        option.dataset.itemName = variant.flavor || 'Standard';
                        option.textContent = `[${variant.branch_name || 'Branch'}] ${variant.flavor || 'Standard'} - ₱${parseFloat(variant.price).toFixed(2)} (Stock: ${variant.available_quantity})`;
            ...
            if (itemSelect) {
                itemSelect.addEventListener('change', function() {
                    ...
                    if (selectedOption && selectedOption.value) {
                        ...
                        itemQuantity.value = Math.min(parseInt(itemQuantity.value) || 1, maxStock);
                        itemStockInfo.innerText = `Max stock: ${maxStock} units`;
                        itemPriceDisplay.innerText = `Price: ₱${price.toFixed(2)}`;
                        branchNote.innerHTML = `<i class="bi bi-shop"></i> Will be fulfilled by: <strong>${branchName}</strong>`;
                    } else {
