                    ...
                    @if(Auth::check() && Auth::user()->barangay)
                        ...
                        @if($userBranch)
                            <span class="btn btn-sm btn-outline-info rounded-pill">
                                <i class="bi bi-shop"></i> Served by: {{ $userBranch->name }}
                            </span>
                        @endif
                    @else
                        <span class="btn btn-sm btn-warning rounded-pill">
                            <i class="bi bi-exclamation-triangle"></i> Update Barangay in Profile
                        </span>
                    @endif
                </div>
    ...
    <div class="mb-5">
        ...
        <div class="row g-4" id="bestSellerGrid">
            @foreach($bestSellers as $bestProduct)
                ...
                @endphp
                @if($bestVariant)
                <div class="col-lg-3 col-md-4 col-6 product-item" data-name="{{ strtolower($bestProduct->name) }}">
                    <div class="card product-card h-100">
                        <div class="position-relative">
                            @if($bestVariant['image'])
                                <img src="{{ Storage::url($bestVariant['image']) }}" class="product-img" alt="{{ $bestProduct->name }}">
                            @else
                            ...
                            @php
                                ...
                                $globalOutOfStock = $groupedProducts[$bestProduct->name]->every(fn($v) => $v['available_quantity'] <= 0);
                            @endphp
                        ...
                        <div class="card-body">
                            ...
                            <div class="small text-muted mt-1">{{ $groupedProducts[$bestProduct->name]->pluck('flavor')->unique()->count() }} variant(s) available</div>
                        </div>
            ...
            <div class="col-lg-3 col-md-4 col-6 product-item" data-name="{{ strtolower($productName) }}">
                <div class="card product-card h-100">
                    ...
                    <div class="card-body">
                        ...
                        <div class="small text-muted mt-1">{{ $variants->pluck('flavor')->unique()->count() }} variant(s) available</div>
                    </div>
                    <div class="card-footer bg-transparent border-0 pb-3">
                        <button class="btn btn-add-cart w-100 text-white select-item-btn"
...
<style>
    .category-badge { font-size: 0.9rem; font-weight: 600; }
    .product-card { transition: transform 0.2s; }
    .product-card:hover { transform: translateY(-5px); box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
    .product-img { height: 200px; width: 100%; object-fit: cover; border-top-left-radius: inherit; border-top-right-radius: inherit; }
    .btn-add-cart { background: #0d6efd; border: none; }
    .btn-add-cart:hover { background: #0b5ed7; }
</style>
        ...
        if (itemModalElement) {
            ...
            itemButtons.forEach(btn => {
                btn.addEventListener('click', async function(e) {
                    ...
                    variants.forEach(function(variant) {
                        ...
                        option.dataset.isUserBranch = variant.branch_id === variant.user_branch_id ? 'true' : 'false';
                        
                        let displayText = (variant.flavor || 'Standard') + ' - ₱' + parseFloat(variant.price).toFixed(2);
                        if(variant.available_quantity <= 0) {
                            displayText += ' (Currently Out of Stock)';
                        } else {
                            displayText += ' (Stock: ' + variant.available_quantity + ')';
                        }
            ...
            itemSelect.addEventListener('change', function() {
                ...
                if (selectedOption && selectedOption.value) {
                    ...
                    itemStockInfo.innerText = maxStock > 0 ? 'Max stock: ' + maxStock : 'Currently Out of Stock';
                    ...
                    if (maxStock <= 0) {
                        ...
                        if (isUserBranch) {
                            fulfilledBranchDisplay.innerText = '✓ Fulfilled by your assigned branch: ' + branchName;
