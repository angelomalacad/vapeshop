...
<div class="container">
    <div class="card shadow-sm border-0">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0">Track Delivery - Order #{{ $order->order_number }}</h4>
        </div>
        <div class="card-body">
            <!-- Status Timeline -->
            <div class="timeline mb-4">
                <div class="d-flex justify-content-between flex-wrap">
                    <div class="text-center {{ in_array($order->order_status, ['pending', 'confirmed', 'processing', 'ready', 'out_for_delivery', 'delivered']) ? 'text-success' : 'text-muted' }}">
                        <i class="bi bi-clock-history fs-3"></i>
                        <div class="small fw-bold">Pending</div>
                        <div class="small">{{ $order->created_at->format('M d, h:i A') }}</div>
                    </div>
                    <div class="text-center {{ in_array($order->order_status, ['confirmed', 'processing', 'ready', 'out_for_delivery', 'delivered']) ? 'text-success' : 'text-muted' }}">
                        <i class="bi bi-check-circle fs-3"></i>
                        <div class="small fw-bold">Confirmed</div>
                    </div>
                    <div class="text-center {{ in_array($order->order_status, ['processing', 'ready', 'out_for_delivery', 'delivered']) ? 'text-success' : 'text-muted' }}">
                        <i class="bi bi-gear fs-3"></i>
                        <div class="small fw-bold">Processing</div>
                    </div>
                    <div class="text-center {{ in_array($order->order_status, ['ready', 'out_for_delivery', 'delivered']) ? 'text-success' : 'text-muted' }}">
                        <i class="bi bi-box-seam fs-3"></i>
                        <div class="small fw-bold">Ready</div>
                    </div>
                    <div class="text-center {{ in_array($order->order_status, ['out_for_delivery', 'delivered']) ? 'text-success' : 'text-muted' }}">
                ...
                <div class="col-md-6">
                    <div class="card bg-light">
                        <div class="card-body">
                            ...
                            @if($order->landmark)<p><strong>Landmark:</strong> {{ $order->landmark }}</p>@endif
                    ...
                    <div class="card">
                        ...
                        <div class="card-body text-center">
                            <a href="{{ Storage::url($delivery->delivery_proof) }}" target="_blank">
                                <img src="{{ Storage::url($delivery->delivery_proof) }}" class="img-fluid rounded" style="max-height: 200px;">
                            </a>
                            <div class="mt-2">
                                <a href="{{ Storage::url($delivery->delivery_proof) }}" target="_blank" class="btn btn-sm btn-outline-primary">View Full Image</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body text-center">
                            <a href="{{ Storage::url($delivery->payment_proof) }}" target="_blank">
                                <img src="{{ Storage::url($delivery->payment_proof) }}" class="img-fluid rounded" style="max-height: 200px;">
                            </a>
                            <div class="mt-2">
                                <a href="{{ Storage::url($delivery->payment_proof) }}" target="_blank" class="btn btn-sm btn-outline-primary">View Full Image</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="mt-3">
                <div class="alert alert-secondary">
                    ...
                    <a href="https://maps.google.com/?q={{ $delivery->driver_latitude }},{{ $delivery->driver_longitude }}" target="_blank">
                        View on Map
                    </a>
                </div>
            </div>
            ...
            <div class="mt-4 text-center">
                <a href="{{ route('customer.orders.show', $order) }}" class="btn btn-secondary rounded-pill">
