    ...
    public function index()
{
    ...
    // ===== ADDED: SALES BY BRANCH (Detailed) =====
$branchSalesData = [];
$branches = Branch::all();
foreach ($branches as $branch) {
    $onlineSales = Order::where('branch_id', $branch->id)
        ->where('order_number', 'NOT LIKE', 'POS-%')
        ->where('payment_status', 'paid')
        ->sum('total_amount');
    $posSales = Order::where('branch_id', $branch->id)
        ->where('order_number', 'LIKE', 'POS-%')
        ->where('payment_status', 'paid')
        ->sum('total_amount');
    $totalSales = $onlineSales + $posSales;
    
    // Only include branches with sales
    if ($totalSales > 0) {
        // Shorten branch name for chart labels
        $shortName = str_replace([' Branch', 'Out Branch'], '', $branch->name);
        $branchSalesData[$shortName] = [
            'total' => $totalSales,
            'online' => $onlineSales,
            'pos' => $posSales,
        ];
    ...
    // ===== ADDED: TOP FLAVORS =====
    $topFlavors = collect();
    if (class_exists('\App\Models\OrderItem') && \Schema::hasColumn('order_items', 'flavor_id')) {
    ...
    if (class_exists('\App\Models\BranchInventory')) {
        $lowStockByBranch = BranchInventory::whereColumn('quantity', '<=', 'low_stock_threshold')
            ->with(['branch', 'product'])
    ...
    return view('admin.dashboard', [
        ...
        'lowStockByBranch' => $lowStockByBranch,
    ]);
}
