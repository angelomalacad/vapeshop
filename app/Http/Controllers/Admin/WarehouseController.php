...
class WarehouseController extends Controller
{
    ...
    {
        $inventory = WarehouseInventory::with('product')->orderBy('product_id')->paginate(20);
        $products = Product::where('is_active', true)->get();
        $lowStockCount = WarehouseInventory::whereColumn('quantity', '<=', 'low_stock_threshold')->count();
        // FIXED: Total value now uses last_purchase_price instead of product price
    ...
    public function addStock(Request $request)
    {
        ...
        $inventory = WarehouseInventory::firstOrNew(['product_id' => $request->product_id]);
        ...
        \App\Models\StockMovement::create([
            ...
            'notes' => "Stock added to warehouse. Purchase price: ₱{$newPurchasePrice}",
            'created_by' => auth()->id(),
        ]);
    ...
    public function approveDistribution(StockTransfer $transfer)
    {
        ...
        $warehouse = WarehouseInventory::where('product_id', $transfer->product_id)->first();
    ...
    {
        $inventory = WarehouseInventory::with('product')->findOrFail($id);
        $products = Product::where('is_active', true)->get();
